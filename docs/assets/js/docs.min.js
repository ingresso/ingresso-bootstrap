 ...
 */
!function(a){"use strict";a(function(){a(".tooltip-demo").tooltip({selector:'[data-toggle="tooltip"]',container:"body"}),a('[data-toggle="popover"]').popover(),a(".tooltip-test").tooltip(),a(".popover-test").popover(),a('.bd-example-indeterminate [type="checkbox"]').prop("indeterminate",!0),a(".bd-example .menu-lk, .bd-example .sld-pg-back").click(function(b){a(this).closest(".sld-pg-lk").toggleClass("active"),a(this).closest(".slide-page").toggleClass("active")}),a('.bd-example [href="#"]').click(function(a){a.preventDefault()});var b=new Swiper(".bd-example .swiper-ingresso-side .gallery-top",{nextButton:".swiper-button-next",prevButton:".swiper-button-prev"}),c=new Swiper(".bd-example .swiper-ingresso-side .gallery-thumbs",{slidesPerView:"auto",direction:"vertical",touchRatio:.2,slideToClickedSlide:!0});b.params.control=c,c.params.control=b,a("#exampleModal").on("show.bs.modal",function(b){var c=a(b.relatedTarget),d=c.data("whatever"),e=a(this);e.find(".modal-title").text("New message to "+d),e.find(".modal-body input").val(d)}),a(".highlight").each(function(){var b='<div class="bd-clipboard"><span class="btn-clipboard" title="Copy to clipboard">Copy</span></div>';a(this).before(b),a(".btn-clipboard").tooltip()});var d=new Clipboard(".btn-clipboard",{target:function(a){return a.parentNode.nextElementSibling}});d.on("success",function(b){a(b.trigger).attr("title","Copied!").tooltip("_fixTitle").tooltip("show").attr("title","Copy to clipboard").tooltip("_fixTitle"),b.clearSelection()}),d.on("error",function(b){var c=/Mac/i.test(navigator.userAgent)?"Press ⌘ to copy":"Press Ctrl-C to copy";a(b.trigger).attr("title",c).tooltip("_fixTitle").tooltip("show").attr("title","Copy to clipboard").tooltip("_fixTitle")})})}(jQuery),function(){"use strict";anchors.options.placement="left",anchors.add(".bd-content > h1, .bd-content > h2, .bd-content > h3, .bd-content > h4, .bd-content > h5")}();